...
type Destination = {
  ...
  posts: { caption: string; media_asset_id: string; media_assets: { storage_path: string; mime_type: string; size_bytes: number; duration_seconds: number | null } | null };
};
...
async function markFailed(client: ReturnType<typeof adminClient>, destination: Destination, code: string, message: string, attemptId?: string) {
  await client.from("post_destinations").update({ status: "failed", last_error_code: code, last_error_message: message.slice(0, 500), updated_at: new Date().toISOString() }).eq("id", destination.id);
  await client.from("posts").update({ status: "failed", updated_at: new Date().toISOString() }).eq("id", destination.post_id);
  if (attemptId) await client.from("publication_attempts").update({ finished_at: new Date().toISOString(), outcome: "permanent_failure", error_category: code }).eq("id", attemptId);
}

async function publishOne(client: ReturnType<typeof adminClient>, destinationId: string) {
  const { data, error } = await client.from("post_destinations")
    .select("id,post_id,social_account_id,platform_options,platform_post_id,posts(caption,media_asset_id,media_assets(storage_path,mime_type,size_bytes,duration_seconds))")
  ...
  const { data: attempts } = await client.from("publication_attempts").select("attempt_number").eq("post_destination_id", destination.id).order("attempt_number", { ascending: false }).limit(1);
  const { data: attempt, error: attemptError } = await client.from("publication_attempts").insert({ post_destination_id: destination.id, attempt_number: (attempts?.[0]?.attempt_number ?? 0) + 1 }).select("id").single();
  if (attemptError || !attempt) throw new Error("Não foi possível registrar a tentativa de publicação.");

  try {
    const media = destination.posts.media_assets;
    ...
    const { data: blob, error: downloadError } = await client.storage.from("post-media").download(media.storage_path);
    ...
    const initResponse = await fetch("https://open.tiktokapis.com/v2/post/publish/video/init/", {
      ...
      body: JSON.stringify({
        post_info: {
          title: destination.posts.caption.slice(0, 2200),
    ...
    await client.from("post_destinations").update({ platform_post_id: initialized.data.publish_id, updated_at: new Date().toISOString() }).eq("id", destination.id);
    ...
    await client.from("publication_attempts").update({ finished_at: new Date().toISOString(), outcome: "succeeded" }).eq("id", attempt.id);
  } catch (error) {
...
async function pollOne(client: ReturnType<typeof adminClient>, destination: Destination) {
  ...
  try {
    ...
    if (result.data.status === "PUBLISH_COMPLETE") {
      ...
      await client.from("post_destinations").update({ status: "published", platform_post_id: platformPostId, published_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", destination.id);
      await client.from("posts").update({ status: "published", updated_at: new Date().toISOString() }).eq("id", destination.post_id);
...
Deno.serve(async (req) => {
  ...
  try {
    const { data: claimed, error } = await client.rpc("claim_due_tiktok_destinations", { batch_size: 3 });
    if (error) throw error;
    for (const item of claimed ?? []) await publishOne(client, item.destination_id);

    const { data: processing } = await client.from("post_destinations")
      .select("id,post_id,social_account_id,platform_options,platform_post_id,posts(caption,media_asset_id,media_assets(storage_path,mime_type,size_bytes,duration_seconds))")
